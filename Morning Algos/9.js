...
function rotateStr(str, amnt) { 
    let answer = str.slice(str.length-amnt) + str.slice(0, str.length-amnt)
    return answer;
}
...
function isRotation(s1, s2) {
    let split = s2.indexOf(s1[0])
    if (s1.length == s2.length && s1.includes(s2.slice(split)) && s1.includes(s2.slice(0, split))){
        return true;
    }
    else {
        return false;
